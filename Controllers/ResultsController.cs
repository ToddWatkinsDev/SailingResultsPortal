    ...
    public class ResultsController : Controller
    {
        ...
        public async Task<IActionResult> Index(string raceId, string classId)
        {
            ...
            var results = allResults
                .Where(r => r.RaceId == raceId && r.ClassId == classId)
        ...
        public async Task<IActionResult> Create(Result result, string finishTimeInput)
        {
            ...
            // Calculate corrected time using async method
            result.CorrectedTime = await CalculateCorrectedTime(result);
        ...
        private async Task<double> CalculateCorrectedTime(Result r)
        {
            var events = await FileStorageHelper.LoadAsync<Event>("events.json");
            var ev = events.FirstOrDefault(e => e.Races.Any(race => race.Id == r.RaceId));
            if (ev == null) return r.FinishTime.TotalSeconds; // fallback

            var race = ev.Races.FirstOrDefault(race => race.Id == r.RaceId);
            var cls = race?.Classes.FirstOrDefault(c => c.Id == r.ClassId);
            if (cls == null) return r.FinishTime.TotalSeconds;

            // Temporary placeholder rating values - replace with real ratings per class
            double rating = cls.ScoringMethod switch
            {
                "Portsmouth" => 1100,     // Example PY number
                "IRC" => 1.05,            // Example IRC rating
                "YTC" => 1.10,            // Example YTC rating
                _ => 1.0
            };

            return cls.ScoringMethod switch
            {
                "Portsmouth" => ScoringService.CalculatePortsmouthCorrectedTime(r.FinishTime, rating),
                "IRC" => ScoringService.CalculateIRCCorrectedTime(r.FinishTime, rating),
                "YTC" => ScoringService.CalculateYTCCorrectedTime(r.FinishTime, rating),
                _ => r.FinishTime.TotalSeconds
            };
        }
    }
}
